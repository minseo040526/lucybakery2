...
with st.sidebar:
    ...
    if st.button('인증하기', disabled=not (consent and phone)):
        uid = upsert_user(phone)
        st.session_state.authed_user_id = uid
        st.success('인증 완료! (개발모드) 맞춤 추천/쿠폰이 활성화됩니다.')
